    ...
    async (filterType: string) => {
      ...
      try {
        ...
        if (data && data.length > 0) {
          ...
          // Fetch shortlist counts for all requests
          ...
          // Map shortlist counts to requests
          if (shortlistCounts) {
            const shortlistMap = new Map(
              shortlistCounts.map((sc: any) => [
                sc.request_id,
                sc.shortlist_count,
              ])
            );
            data.forEach((request: any) => {
              request.shortlist_count = shortlistMap.get(request.id) || 0;
            });
          }
        }
