-- Create a junction table to allow multiple CSRs to shortlist the same request
-- This replaces the single shortlisted_by column with a many-to-many relationship
...
CREATE TABLE IF NOT EXISTS request_shortlists (
  ...
  UNIQUE(request_id, csr_id) -- Prevent duplicate shortlists from same CSR
);
...
-- If a request has shortlisted=true and shortlisted_by is set, migrate it
...
-- Create a function to get shortlist count for requests (safe for users to call)
...
-- Note: We keep the old shortlisted and shortlisted_by columns for backward compatibility
-- but they will be phased out in favor of the new junction table
